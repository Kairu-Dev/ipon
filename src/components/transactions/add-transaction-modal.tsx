    ...
    const timer = setTimeout(async () => {
      if (note && note.length > 5) {
        try {
          ...
          console.log("Raw AI Suggestion returned to frontend:", suggestion);
          if (suggestion) {
            setAiSuggestion(suggestion);
          }
        } catch (error) {
          console.error("AI Suggestion Error:", error);
          // Fail silently in UI as per requirements, but log for debugging
        }
      } else {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col flex-1 overflow-hidden">
          <div className="p-6 overflow-y-auto space-y-8 flex-1">
            ...
            <div className="flex flex-col items-center justify-center py-4 space-y-2">
              <p className="font-label-xs text-secondary uppercase tracking-wider">Amount</p>
              ...
              <div className="flex justify-between items-end">
                ...
                {aiSuggestion && aiSuggestion !== category && (
                  ...
                  >
                    ...
                    <span className="font-label-xs">AI suggests: {categories.find(c => c.value === aiSuggestion)?.label || "..."} (Click to apply)</span>
                  </button>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label htmlFor="paymentMethod" className="font-label-md text-on-surface">Payment Method</label>
                <select 
                  id="paymentMethod"
